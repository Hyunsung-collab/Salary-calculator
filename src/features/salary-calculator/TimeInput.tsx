...
import { isValidTime } from "@/lib/workTimeValidation"
...
export function TimeInputModeControl({ mode, onChange }: { mode: TimeInputMode; onChange: (mode: TimeInputMode) => void }) {
  return (
    <fieldset className="space-y-2">
      <legend className="mb-2 text-sm font-medium">시간 입력 방법</legend>
      <div className="flex gap-2">
        ...
        <Button type="button" className="min-h-11 flex-1 sm:flex-none" variant={mode === "text" ? "default" : "outline"} aria-pressed={mode === "text"} onClick={() => onChange("text")}>직접 입력</Button>
      </div>
      <p className="text-sm text-slate-500">{mode === "select" ? "시와 분을 선택해 주세요." : "24시간 기준으로 입력해 주세요. 예: 09:00, 18:30"}</p>
    </fieldset>
  )
}
...
export function TimeInput({ label, value, onChange, mode, error }: {
  ...
  const message = error || (!valid && value ? "시간을 00:00~23:59 형식으로 입력해 주세요." : undefined)
  return (
    <fieldset className="min-w-0 space-y-2">
      <legend className="text-sm font-medium text-slate-700">{label}</legend>
      {mode === "text" ? (
        <Input aria-label={label} type="text" inputMode="text" autoComplete="off" placeholder="09:00" value={value} onChange={(event) => onChange(event.target.value)} aria-invalid={Boolean(message)} aria-describedby={message ? id : undefined} className="min-h-11 min-w-0 tabular-nums" />
      ) : (
