...
export function SalaryHome({
  ...
  if (showGuidedSetup && setupStep !== "result") {
    return (
        ...
        <section aria-labelledby="guided-setup-title" className="space-y-5">
          <header className="space-y-2">
            ...
            <h1 id="guided-setup-title" className="text-2xl font-semibold tracking-tight text-slate-900">
              {setupStep === "settings" ? "이번 달 급여를 계산해볼게요" : "이번 달 근무를 알려주세요"}
            </h1>
            <p className="max-w-2xl text-sm leading-6 text-slate-600">
              {setupStep === "settings" ? "먼저 급여 조건을 알려주세요." : "근무 요일과 시간을 입력하면 예상 급여를 바로 확인할 수 있어요."}
            </p>
          </header>
          ...
          {setupStep === "settings" && (
            <Card>
              <CardHeader>
                <CardTitle>급여 조건 설정</CardTitle>
              </CardHeader>
              <CardContent className="space-y-5">
                <SalarySettingsForm settings={setupSettings} onChange={onSetupSettingsChange} onValidityChange={setSettingsValid} />
                <div className="flex justify-end border-t border-slate-100 pt-4">
                  <Button type="button" className="min-h-12 w-full sm:w-auto" disabled={!settingsValid} onClick={onConfirmSetupSettings}>
                    근무 입력으로 계속
                  </Button>
                </div>
              </CardContent>
  ...
  return (
    <div className="space-y-6">
      ...
      <WorkCalendar key={selectedMonth} selectedMonth={selectedMonth} entries={entries} onGoToWork={onGoToWork} />
